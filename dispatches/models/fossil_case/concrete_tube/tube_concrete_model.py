...
class ConcreteTESData(UnitModelBlockData):
    ...
    def build(self):
        ...
        if "inlet_temperature_charge" in data.keys():
            inlet_enthalpy_charge = iapws95.htpx(data["inlet_temperature_charge"] * pyunits.K,
                                          data["inlet_pressure_charge"] * pyunits.Pa)

        elif "inlet_enthalpy_charge" in data.keys():
            inlet_enthalpy_charge = data["inlet_enthalpy_charge"]

        else:
            raise Exception(
                "The concrete TES Input data must contain either inlet_enthalpy or inlet_temperature. "
                "If both are provided, inlet_temperature is used")
        # for discharge
        if "inlet_temperature_discharge" in data.keys():
            inlet_enthalpy_discharge = iapws95.htpx(data["inlet_temperature_discharge"] * pyunits.K,
                                          data["inlet_pressure_discharge"] * pyunits.Pa)

        elif "inlet_enthalpy_discharge" in data.keys():
            inlet_enthalpy_discharge = data["inlet_enthalpy_charge"]
        ...
        for p in self.time_periods:
            ...
            self.period[p].tube_charge.tube_inlet.flow_mol[0].fix(data["flow_mol_charge"])
            self.period[p].tube_charge.tube_inlet.pressure[0].fix(data["inlet_pressure_charge"])
            self.period[p].tube_charge.tube_inlet.enth_mol[0].fix(inlet_enthalpy_charge)
            self.period[p].tube_charge.tube.deltaP.fix(data['deltaP'])

            self.period[p].tube_discharge.tube_inlet.flow_mol[0].fix(data["flow_mol_discharge"])
            self.period[p].tube_discharge.tube_inlet.pressure[0].fix(data["inlet_pressure_discharge"])
            self.period[p].tube_discharge.tube_inlet.enth_mol[0].fix(inlet_enthalpy_discharge)
